...
- **MCP substrate reach (v5.5.0 + v5.7.0).** Five new 5-minute mount guides + smoke tests land alongside the Phase-1 flagships ([Cursor](docs/integrations/cursor.md) + [OpenAI Agents SDK](docs/integrations/openai-agents-sdk.md)): [Claude Desktop](docs/integrations/claude-desktop.md), [Continue](docs/integrations/continue.md), [Windsurf](docs/integrations/windsurf.md), [Zed](docs/integrations/zed.md), and now [Gemini CLI](docs/integrations/gemini-cli.md). One `mnemo mcp` stdio server, seven documented hosts, one tool surface — `docs/integrations/PICKS.md` is the rubric for the next pick.
