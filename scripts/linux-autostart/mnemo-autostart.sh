#!/usr/bin/env bash
# mnemo-autostart.sh - production wrapper for the mnemo daemon autostart (Linux).
#
# v5.10.0 mirrors the v5.8.1 Windows Task Scheduler wrapper. systemd
# treats exit 0 as success and exit non-zero as failure (and the
# Restart=on-failure directive will respawn the unit automatically).
# The wrapper:
#
...
#      ``${XDG_STATE_HOME:-$HOME/.local/state}/mnemo/logs/autostart.log``
#      so future debugging has evidence the autostart fired.
#
# Intentionally a separate script (not inlined into the unit's
# ExecStart=) so it's editable + version-controlled + testable
...
LOG_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/mnemo/logs"
